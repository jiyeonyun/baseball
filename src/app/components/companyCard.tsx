...
export default function CompanyCard({ data }: any) {
    ...
    const classNames = {
        wrapper: "p-4 rounded-xl shadow-md bg-white/50 dark:bg-slate-800/80",
        header: "flex items-center justify-between",
        url: "text-blue-500 dark:text-blue-400 text-sm",
        tagWrap: "flex flex-wrap gap-2 mt-2",
        tag: "bg-gray-200 dark:bg-slate-600 dark:text-slate-200 px-2 py-1 rounded-lg text-sm",
        period: "text-sm text-gray-500 dark:text-gray-400 mt-1",
        tasks: "mt-2 list-disc ml-5 text-sm",
        tasksItem: "text-gray-700",
        detailBtn: "text-blue-600 dark:text-blue-400 cursor-pointer hover:transition duration-300",
        ...
        <>
            <div className={classNames.wrapper} style={containerStyle}>
                <div className={classNames.header}>
                    <div>
                        <h2 className={classNames.title}>{data.name}</h2>
                        ...
                        <ul className={classNames.tasks}>
                            {data.tasks.map((task: any, i: number) => (
                                <li className={classNames.tasksItem} key={i}>
                                    {task}
                                </li>
                            ))}
                        </ul>
